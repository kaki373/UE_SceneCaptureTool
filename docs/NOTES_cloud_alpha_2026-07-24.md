...
- **映像 Behind に手前オブジェクトが不透明で写る（ユーザー報告）**: 映像はメインジョブの
  MatteSil PP パスをマスクにしていたが、**ObjectID 対象とカスタムステンシルバッファを
  共有**するため、ObjectID 対象（BG_Lo は結合メッシュでバウンズが板より手前まで届く）が
  板より近い画素でステンシルが上書きされシルエットに穴が開き、合成が Beauty 側
  （手前オブジェクト入り）を選んでいた。対策: 静止画側と同一手法に統一 —
  レンダ後に各フレームをシーケンサーでスクラブ評価し、show-only 深度の SceneCapture で
  板の全投影シルエットを生成（`core.render_matte_sil_sequence`）。映像ジョブから
  MatteSil PP パスを廃止＝衝突が構造的に不可能。
  ※途中案の「プレートジョブに MatteSil を同居（板を matte レンダモード）」は不成立:
  **render_in_main_pass=False でも板は深度で奥を遮る**（プレートの空が黒くなる・実測。
  Beauty の「黒空」ルックもこの機構）。
- **BackingT 書き出し中にウインドウが連続開閉して UI 操作不能（ユーザー報告）**:
  フレーム毎の ffmpeg 起動がコンソールウインドウを出していた。subprocess に
  CREATE_NO_WINDOW (0x08000000) を付与（MP4 エンコード側は元々付いていた）。
- BehindPlate の near-clip 距離はカメラが動く前提で開始/中間/終端フレームをサンプルした
  最小値に変更（従来は開始フレームのみ）。

### 追補2（2026-07-25）: 白1レンダ化・VDBセレクタ・レビュー修正
- **黒レンダ廃止**: 黒側は「手前の内容の発光C」を差し引くための測定だったが、白を
  発光100にすると C/E ≤ 1% で無視できる → 白1レンダで T = W/素板レベル（ジョブ1本減）。
  前提として2つの実測バグ修正が必須だった: ① **MRQ の EXR は既定でトーンカーブ適用済み**
  （発光100が~1.0に圧縮）→ バッキングジョブに MoviePipelineColorSetting
  disable_tone_curve=True。② gray16 変換は 1.0 クリップ+リミテッドレンジ →
  ffmpeg `format=gbrpf32le,extractplanes=g` で G チャンネルを PFM(float32) 抽出
  （ビット一致・>1.0 保持）。※つまり従来の白黒差分も非線形データで計算しており
  雲αをわずかに過大充填していた（リニア化で解消）。
- **UI に「VDB雲: Auto / ON / OFF」セレクタ**（両タブ共通・既定 Auto=HV検出で自動）。
  OFF は雲を完全無視した従来書き出し（雲対象は注記を出して無視）。
- **コードレビュー（2レビュアー+自己）でエラー経路の欠陥8件を修正**: 静止画バッキング
  失敗時に MatteBeauty が未合成 / 映像バッキング起動例外で _finish_outputs 二重呼び出し /
  起動例外で グローバル near-clip 残留 / Behind 失敗でメイン素材の後処理放棄 /
  静止画チェーン死亡 / PFM 一時ファイルリーク / 連番の固定スケールが失敗フレームで
  再基準化 / 手前雲判定を中心→バウンズ最近点へ。正常系はスモークテストでバイト一致。
