...
- **雲マット**: レベル内の全 VDB雲（HeterogeneousVolume）だけの白黒マスク（**雲=黒 / 背景=白**・ソフトエッジ・**深度順序どおり**＝可視雲モード（白バッキング差分の一般化。雲を貫くオブジェクトはマットに乗らず、手前の雲は物体を覆う））。PIE 側で全ジオメトリを発光100のアンリット白材へ差し替え、何も隠さず1本の EXR ジョブ＋GeoMask パスから合成する。UDS の大気雲レイヤー（VolumetricCloud）は対象外（エンジン制約）。I2I の雲領域マスク用。両タブ対応（映像は PNG連番/MP4、雲マットのみ選択ならメインレンダ省略）。
