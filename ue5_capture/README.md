...
| **Depth** | 16bit/8bit PNG or EXR | カメラからの距離(**cm**)。Near/Far 正規化、`手前=白/奥=黒`反転可。EXR は生cm。Normal と共有の可視雲αジョブで雲領域を黒（=奥）に（`depth'=depth×(1−雲α)`・**8bit/16bit PNG のみ、EXR float は対象外**＝生cmは 0=カメラ位置になり意味が壊れるため） | SceneCapture |
...
| **CloudMatte** | 白黒 RGB PNG | レベル内の全 VDB雲のマスク（雲=黒/背景=白・ソフトエッジ・**深度順序どおりの可視雲**）。可視雲モード（黒バッキング輝度 EXR + GeoMask）から合成 | MRQ(可視雲ジョブ) |
...
- **雲のみが Matte 対象（板なし）のとき**: 従来の**分離モード CloudMatte ジョブ**
  （雲以外を隠して α レンダ・遮蔽なし全投影・完了メッセージに注記）。
  分離モードで残すライトは**太陽（Directional）/スカイライト持ちのみ**（雲の照明は
  太陽/スカイ支配。ローカルライトしか持たないアクターを残すと不透明ジオメトリが
  α=1 でマスクを汚す・AV024 実測 2026-08-25）。さらに **LevelInstance 内包アクターと
  Sequencer スポーナブルはエディタ側の hidden が PIE に伝搬しない**（PIE で資産から
  再生成される）ため、PIE 開始後に PIE ワールド側で毎 tick 冪等に隠す
  （`_start_pie_cloud_hider`。PIE は破棄されるので復元不要）。
- **可視雲モード（雲マット・Normal/Depthの雲抜き、2026-08-26 更新：黒バッキング輝度方式）**:
  分離モード（雲以外を隠す・遮蔽なし全投影）ではなく、**PIE 側で全ジオメトリを純黒
  アンリット材（`M_UE5Cap_MatteBoardUnlit`）へ差し替える**（LevelInstance 内包アクター・
  Sequencer スポーナブルも対象。エディタ側のプロパティ変更が届かない、PIE がアセットから
  再生成するアクターにも効く PIE 側ウォッチャー）。無影フィルライト **20 lux×3** で
  雲だけを光らせ、黒背景に浮かぶ雲の散乱輝度 **L（＝見えている雲量＝知覚的カバレッジ）**
  を測る（白バッキングの 1−T は物理透過率で Beauty の見た目より薄くなる実測があったための
  変更）。何も隠さず1本の EXR ジョブで RGB=L と α=分離モード相当の全投影カバレッジを
  同時取得し、同ジョブに「GeoMask」追加ポストプロセスパス（SceneDepth<1e7→白）を載せる。
  合成（`capture_core.compose_visible_cloud`）は空画素に正確な α を、ジオメトリ画素に
  L×scale を採用する（**自己較正**：空に雲がある構図は空の雲画素で `scale=median(α/L)`
  を算出、空に雲が無い構図は輝度 p99 で代用）。これを飽和カーブ `1−e^(−G·x)`
  （`_VIS_CLOUD_GAIN`、既定 G=20）に通して内部を不透明化する——雲の消衰で内部輝度が
  落ち「輪郭は合うが中身が薄い」マットになる実測への対処（実測：G=6でコア94%／
  G=12で98.6%／G=20で99.7%／G=30でほぼ二値。柱を貫く筋は輝度≈0のためカーブ後も残る）。
  **深度順序どおりなので雲を貫くオブジェクトはマットに乗らず、手前の雲は物体を
  覆う**。UE の EXR α は 1−カバレッジで格納されるため読み込み時に反転して補正する。
  最終成果物は Matteの前と同じく雲=黒/背景=白へ反転して出力。雲マット（両タブ）と
  Normal / Depth の雲抜きはこの1ジョブを共用する（Depth は `depth'=depth×(1−雲α)` で
  適用。8bit/16bit PNG のみ、EXR float は対象外）。映像タブでは EXR 連番
  （CloudMatte+GeoMask）をレンダし、`compose_visible_cloud_sequence` でフレーム毎に
  合成後 EXR を削除する。
- **可視雲ジョブの無影フィルライト**: HV の輝度・α は照明依存で、照明の弱い遠景の雲は
  暗く沈み L（雲量の信号）や α から消える（AV024 実測）。可視雲ジョブの間だけ低照度
  （**20 lux×3・下向き+斜め2方向・無影**）のフィルライトを一時スポーンして雲の散乱輝度と
  HV alpha を安定させる。バッキングレンダと同条件で露出適応/ローカル露出/ブルーム/Lumen
  スクリーントレースを無効化しトーンカーブも切る。レンダ後に破棄。
- **分離モードは Matteの前 の内部雲ジョブに限定**: 板対象が無く雲のみが Matte 対象
  のときの内部雲ジョブ（上記「雲のみが Matte 対象（板なし）のとき」）は従来どおり
  分離モード（雲以外を隠す・遮蔽なし全投影）のまま。映像レンダで雲マットジョブと
  共用される場合も、共用先が分離モードなら遮蔽なし・完了メッセージに注記が出る。
