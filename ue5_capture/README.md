...
| **CloudMatte** | 白黒 RGB PNG | レベル内の全 VDB雲だけのマスク（**雲=黒/背景=白**・ソフトエッジ・遮蔽なし全投影）。αレンダ後に反転白黒化 | MRQ(雲αジョブ) |
...
- **雲マット独立出力（両タブの「雲マット」・2026-08-25 追加）**: 上記の分離モード
  CloudMatte を独立素材に昇格したもの。対象はレベル内の全 VDB雲（Matteの前の
  雲ジョブと対象が同じなら 1 本で共用）。α を**反転**白黒 RGB（雲=黒/背景=白）へ
  変換して出力するので MP4 エンコード可能。映像タブで雲マットのみ選択なら
  メインジョブを省略する。
- **雲αジョブの無影フィルライト（2026-08-26 追加）**: HV の α は照明依存で、
  照明の弱い遠景の雲が α から消える（AV024 実測・無照明で α0 の既知実測と同根）。
  分離モードの間だけ無影の DirectionalLight 3灯（下向き+斜め2方向・
  atmosphere_sun_light OFF）を一時スポーンして全雲を照らす。RGB は捨てるので
  過露光は無害。レンダ後に破棄。
- **Normal の雲抜き（2026-08-26 追加）**: Normal 選択時は雲αを自動レンダし、
  Normal へ (1−α) を乗算して雲領域を黒に落とす（雲は GBuffer 法線を持たず、
  放置すると雲の奥のジオメトリの法線が透けて写るため）。
