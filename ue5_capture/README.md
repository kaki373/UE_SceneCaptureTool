...
| **CloudMatte** | 白黒 RGB PNG | レベル内の全 VDB雲だけのマスク（雲=白・ソフトエッジ・遮蔽なし全投影）。αレンダ後に白黒化 | MRQ(雲αジョブ) |
...
- **雲マット独立出力（両タブの「雲マット」・2026-08-25 追加）**: 上記の分離モード
  ...
  雲ジョブと対象が同じなら 1 本で共用）。α を白黒 RGB へ変換して出力するので
  MP4 エンコード可能。映像タブで雲マットのみ選択ならメインジョブを省略する。
