...
| **CloudMatte** | 白黒 RGB PNG | レベル内の全 VDB雲のマスク（雲=黒/背景=白・ソフトエッジ・**深度順序どおりの可視雲**）。可視雲モード（白バッキング化 EXR + GeoMask）から合成 | MRQ(可視雲ジョブ) |
...
- **可視雲モード（雲マット・Normalの雲抜き、2026-08-26 更新）**: 分離モード（雲以外を
  隠す・遮蔽なし全投影）ではなく、**PIE 側で全ジオメトリを発光100のアンリット白材へ
  差し替える**（LevelInstance 内包アクター・Sequencer スポーナブルも対象。エディタ側の
  プロパティ変更が届かない、PIE がアセットから再生成するアクターにも効く PIE 側
  ウォッチャー）。何も隠さず1本の EXR ジョブで RGB=W=T×素板レベル（T=手前の雲の
  透過率・**深度順序どおり**）と α=分離モード相当の全投影カバレッジを同時取得し、
  同ジョブに「GeoMask」追加ポストプロセスパス（SceneDepth<1e7→白）を載せる。合成
  （`capture_core.compose_visible_cloud`）はジオメトリ画素に 1−T、空画素に α を採用
  ...
  Normal の雲抜きはこの1ジョブを共用する。映像タブでは EXR 連番（CloudMatte+GeoMask）
  をレンダし、`compose_visible_cloud_sequence` でフレーム毎に合成後 EXR を削除する。
- **可視雲ジョブの無影フィルライト**: HV の α は照明依存で、照明の弱い遠景の雲が
  α から消える（AV024 実測）。可視雲ジョブの間だけ低照度（**5 lux×3・下向き+斜め2方向・
  無影**）のフィルライトを一時スポーンして HV alpha を安定させる。RGB は捨てるので
  過露光は無害。バッキングレンダと同条件で露出適応/ローカル露出/ブルーム/Lumen
